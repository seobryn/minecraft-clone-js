...
export class WorldChunk extends THREE.Group {
  ...
  #generateResources (rng) {
    ...
    const { width, height } = this.size
    resourcesBlocks.forEach((resource) => {
      for (let x = 0; x < width; x += 1) {
        for (let y = 0; y < height; y += 1) {
          for (let z = 0; z < width; z += 1) {
    ...
    for (let x = 0; x < width; x++) {
      for (let z = 0; z < width; z++) {
        ...
        for (let y = 0; y <= height; y++) {
          if ((y < _height && this.getBlockAt(x, y, z).id === blocks.empty.id) || y === 0) {
            this.setBlockId(
              { x, y, z },
              blocks.dirt.id
            )
          } else if (y < _height && this.getBlockAt(x, y, z).id === blocks.cave.id && y > 0) {
            this.setBlockId(
              { x, y, z },
              blocks.cave.id
            )
    ...
    for (let x = 0; x < width; x += 1) {
      for (let y = 0; y < height; y += 1) {
        for (let z = 0; z < width; z += 1) {
          ...
          if (blockId === blocks.empty.id || blockId === blocks.cave.id) continue
